...
export function useCandidateModals() {
    ...
    (selectedIndustries: string[]) => {
      ...
      if (modalState.targetIndex !== null) {
        if (modalState.targetIndex === -1) {
          ...
          modalState.targetIndex >= 1000 &&
          modalState.targetIndex < 2000
        ) {
          // 職務経歴エントリの業界を更新
          const entryIndex = modalState.targetIndex - 1000;
          context.updateWorkHistoryEntry(
            entryIndex,
            'industries',
            selectedIndustries
          );
          ...
          context.updateSelectedIndustries(
            modalState.targetIndex,
            selectedIndustries
          );
        }
      }
  ...
  // 職種選択モーダルの処理
  const handleJobTypeConfirm = useCallback(
  ...
  // 働き方選択モーダルの処理
  const handleWorkStyleConfirm = useCallback(
  ...
  // 働き方選択モーダルを開く
  const openWorkStyleModal = useCallback(
    (targetIndex: number) => {
  ...
  // 選択された業界の削除
  const removeIndustryFromSelection = useCallback(
    (entryIndex: number, industry: string) => {
  ...
  // 職歴業界の削除
  const removeRecentJobIndustry = useCallback(
    (industryIndex: number) => {
      const { formData } = context;
      const newIndustries = formData.recentJobIndustries.filter(
        (_, i) => i !== industryIndex
      );
  ...
  // 職歴職種の削除
  const removeRecentJobType = useCallback(
    (jobTypeIndex: number) => {
      const { formData } = context;
      const newJobTypes = formData.recentJobTypes.filter(
        (_, i) => i !== jobTypeIndex
      );
  ...
  const getModalInitialData = useCallback(
    (modalType: 'industry' | 'jobtype' | 'workstyle', targetIndex: number) => {
      const { formData, workHistoryEntries } = context;
      ...
      if (modalType === 'industry') {
        if (targetIndex === -1 || targetIndex === -2) {
          ...
          return workHistoryEntries[entryIndex]?.industries || [];
        } else if (
          targetIndex >= 0 &&
          targetIndex < context.careerStatusEntries.length
        ) {
        ...
        if (targetIndex === -1 || targetIndex === -2) {
          ...
          return workHistoryEntries[entryIndex]?.jobTypes || [];
        }
