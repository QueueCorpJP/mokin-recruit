...
const JobTypeSelectModal = React.memo(
  function JobTypeSelectModal({
    isOpen,
    onClose,
    onConfirm,
    initialSelected = [],
  ...
  (prevProps, nextProps) => {
    // 再レンダリングが必要な場合のみfalseを返す
    if (prevProps.isOpen !== nextProps.isOpen) return false;
    if (prevProps.maxSelections !== nextProps.maxSelections) return false;

    // モーダルが閉じている時は、initialSelectedの変更を無視
    if (!nextProps.isOpen) return true;

    // モーダルが開いている時は、initialSelectedの変更もチェック
    return (
      JSON.stringify(prevProps.initialSelected) ===
      JSON.stringify(nextProps.initialSelected)
    );
  }
);

export default JobTypeSelectModal;
