...
export function MessageLayout({
  ...
  const [searchTarget, setSearchTarget] = useState<'company' | 'job'>(
    'company'
  );

  const handleSearchTargetChange = (target: 'company' | 'job') => {
    console.log(
      'MessageLayout - searchTarget changing from',
      searchTarget,
      'to',
      target
    );
  ...
  const filteredMessages = (messages || []).filter(message => {
    if (isCandidatePage) {
      // 特定のroomが選択されている場合
      if (companyFilter !== 'all' && message.id !== companyFilter) return false;
        ...
        <div className='flex-1 min-h-0 flex flex-col h-full'>
          ...
          <div className='flex-1 overflow-y-auto scrollbar-hide'>
            {(() => {
              ...
              return Boolean(
                selectedMessageId && chatMessages && chatMessages.length > 0
              );
            })() ? (
