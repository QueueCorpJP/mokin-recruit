...
export const NewsTabsAndList: React.FC<NewsTabsAndListProps> = ({ 
  ...
  const filteredArticles = activeTab === 'すべて' 
    ? articles 
          ...
          >
            ...
            <div className="news-card-inner bg-[#ffffff] flex flex-col gap-4 flex-grow items-start justify-start p-[24px] relative">
              <div className="flex items-start justify-between relative w-full">
                  ...
                  {article.categories && article.categories.length > 0 && (
                    <>
                      {article.categories.slice(0, 2).map((category, index) => (
                        <div key={index} className="bg-[#0f9058] flex gap-2.5 items-center justify-center px-4 py-1 relative rounded-[28px]">
                          <div className="font-bold text-[#ffffff] text-[14px] whitespace-nowrap tracking-[1.4px]" style={{ fontFamily: 'Noto Sans JP, sans-serif' }}>
                ...
                <div className="md:block hidden font-medium text-[#999999] text-[14px] whitespace-nowrap tracking-[1.4px] flex-shrink-0" style={{ fontFamily: 'Noto Sans JP, sans-serif' }}>
                  <p className="leading-[1.6]">{article.date}</p>
                </div>
              </div>
              <div className="font-bold h-[60px] overflow-hidden text-[#323232] text-[18px] tracking-[1.8px] w-full prose-lists" style={{ 
              ...
              <div className="block md:hidden font-medium text-[#999999] text-[14px] whitespace-nowrap tracking-[1.4px] flex-shrink-0" style={{ fontFamily: 'Noto Sans JP, sans-serif' }}>
                <p className="leading-[1.6]">{article.date}</p>
              </div>
            </div>
          </div>
