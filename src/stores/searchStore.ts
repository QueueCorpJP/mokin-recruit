...
export const useSearchStore = create<SearchState>()(
  subscribeWithSelector((set, get) => ({
    ...
    setIsDesiredJobTypeModalOpen: (open) => set({ isDesiredJobTypeModalOpen: open }),
    ...
    getSearchParams: () => {
      ...
      if (state.experienceJobTypes.length > 0) {
        params.set('experience_job_types', state.experienceJobTypes.map(j => j.name).join(','));
      }
      if (state.experienceIndustries.length > 0) {
        params.set('experience_industries', state.experienceIndustries.map(i => i.name).join(','));
      }
      
      if (state.currentSalaryMin) params.set('current_salary_min', state.currentSalaryMin);
      if (state.currentSalaryMax) params.set('current_salary_max', state.currentSalaryMax);
      if (state.currentCompany) params.set('current_company', state.currentCompany);
      
      if (state.education) params.set('education', state.education);
      if (state.englishLevel) params.set('english_level', state.englishLevel);
      if (state.otherLanguage) params.set('other_language', state.otherLanguage);
      if (state.otherLanguageLevel) params.set('other_language_level', state.otherLanguageLevel);
      if (state.qualifications) params.set('qualifications', state.qualifications);
      ...
      if (state.desiredJobTypes.length > 0) {
        params.set('desired_job_types', state.desiredJobTypes.map(j => j.name).join(','));
      }
      if (state.desiredIndustries.length > 0) {
        params.set('desired_industries', state.desiredIndustries.map(i => i.name).join(','));
      }
      if (state.desiredSalaryMin) params.set('desired_salary_min', state.desiredSalaryMin);
      if (state.desiredSalaryMax) params.set('desired_salary_max', state.desiredSalaryMax);
      if (state.desiredLocations.length > 0) {
        params.set('desired_locations', state.desiredLocations.map(l => l.name).join(','));
      }
      ...
      if (state.selectionStatus) params.set('selection_status', state.selectionStatus);
      if (state.similarCompanyIndustry) params.set('similar_company_industry', state.similarCompanyIndustry);
      if (state.similarCompanyLocation) params.set('similar_company_location', state.similarCompanyLocation);
