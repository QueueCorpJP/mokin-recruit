...
function normalizeConditions(raw: any): any {
  ...
  if (raw.desired_job_types && !raw.job_types) normalized.job_types = raw.desired_job_types;
  if (raw.desired_industries && !raw.industries) normalized.industries = raw.desired_industries;
  if (raw.desired_locations && !raw.locations) normalized.locations = raw.desired_locations;
  if (raw.education_levels && !raw.education_level) normalized.education_level = raw.education_levels;
  if (raw.language_skills && !raw.languages) normalized.languages = raw.language_skills;

  // 年齢・年収などの数値は安全に数値化（不正なら未設定扱い）
  const toNum = (v: any) => (typeof v === 'string' ? parseInt(v, 10) : typeof v === 'number' ? v : undefined);
...
export function SavedSearchRecommendationsClient({ companyGroupId, jobOptions = [], initialSavedSearches = [] }: Props) {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [savedSearches, setSavedSearches] = useState<any[]>(initialSavedSearches);
    ...
    const fetchAll = async () => {
      try {
        ...
        if (!initialSavedSearches.length) {
          ...
          if (shError) {
            console.error('[SavedSearchRecommendationsClient] search_history error:', shError);
          }

          const saved = (histories || []).filter((h: any) => {
            const v = h.is_saved;
            return v === true || v === 'true' || v === 'TRUE' || String(v).toLowerCase() === 'true';
          }).slice(0, 3);
        ...
        if (cError) {
          console.error('[SavedSearchRecommendationsClient] candidates error:', cError);
          if (mounted) setCandidates([]);
        } else {
          const mapped: CandidateData[] = (rows || []).map((candidate: any, index: number) => {
            ...
            const lastLogin = candidate.last_login_at
              ? new Date(candidate.last_login_at).toLocaleDateString('ja-JP', { year: 'numeric', month: 'numeric', day: 'numeric' })
              : '未ログイン';
            ...
            return {
              ...
              age: age ? `${age}歳` : '年齢未設定',
              gender:
                candidate.gender === 'male' ? '男性' : candidate.gender === 'female' ? '女性' : '未設定',
              salary: candidate.desired_salary || '未設定',
              university: '未設定',
              degree: '未設定',
              experienceJobs: (candidate.desired_job_types || candidate.skills || []).slice(0, 3),
        ...
        console.error('[SavedSearchRecommendationsClient] unexpected error:', e);
  ...
  const sections = useMemo(() => {
    return savedSearches.map((history) => {
      ...
      let matched = searchCandidatesWithMockData(conditions, candidates).slice(0, 3);
      if (!matched.length) {
    ...
    return (
      <div className="bg-[#EFEFEF] p-6 rounded-[24px] text-center">
        <p
          ...
          style={{ fontFamily: 'Noto Sans JP, sans-serif', letterSpacing: '0.04em', lineHeight: 1.6 }}
        >
          現在おすすめの候補者はいません。
        </p>
      </div>
      ...
      {sections.map((sec) => (
        <RecommendedCandidatesSection
          ...
          searchCondition={{
            ...
            title: sec.title,
            conditions: savedSearches.find(s => s.id === sec.id)?.search_conditions || {},
          }}
