            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {searchStore.experienceJobTypes.filter(job => job && job.id).map((job) => (
                <SelectableTagWithYears
                  ...
                  onYearsChange={(id, years) => searchStore.updateExperienceJobTypeYears(id, years)}
                  onRemove={(id) => {
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {searchStore.experienceIndustries.filter(industry => industry && industry.id).map((industry) => (
                <SelectableTagWithYears
                  ...
                  onYearsChange={(id, years) => searchStore.updateExperienceIndustryYears(id, years)}
                  onRemove={(id) => {
                    searchStore.setExperienceIndustries(
                      searchStore.experienceIndustries.filter((i) => i.id !== id)
                    );
                  }}
