...
export async function getCompanyGroups() {
  try {
    ...
    const { data: userPermissions, error } = await supabase
      ...
      .eq('company_user_id', (companyUser as { id: string }).id);
...
export function loadSearchParamsToStore(
  ...
  if (searchParams.get('desired_job_types')) {
    const desiredJobTypes = searchParams
  ...
  if (searchParams.get('desired_industries')) {
    const desiredIndustries = searchParams
  ...
  if (searchParams.get('desired_locations')) {
    const desiredLocations = searchParams
