...
export default function SearchClient({
  ...
  const filteredCandidates = useMemo(() => {
    ...
    return candidates.filter(candidate => {
      ...
      if (filters.newUser) {
        if (
          !candidate.lastLogin.includes('日前') &&
          !candidate.lastLogin.includes('時間前')
        ) {
          return false;
        }
      }
      ...
      if (filters.lastLogin) {
        if (
          !candidate.lastLogin.includes('時間前') &&
          !candidate.lastLogin.includes('1日前')
        ) {
          return false;
        }
      }
