...
export async function sendVerificationCode(email: string) {
  try {
    ...
    if (process.env.NODE_ENV === 'development') console.log('認証成功 - ユーザーID:', authResult.data.companyUserId);
    ...
    if (process.env.NODE_ENV === 'development') console.log('認証コードをpassword_reset_tokensテーブルに保存中...');
...
export async function verifyCode(code: string) {
  try {
    ...
    if (process.env.NODE_ENV === 'development') console.log('認証コードをpassword_reset_tokensテーブルから検索中...');
    ...
    if (!newEmail) {
      if (process.env.NODE_ENV === 'development') console.error('新しいメールアドレスの抽出に失敗:', verification.email);
    ...
    if (process.env.NODE_ENV === 'development') safeLog('debug', 'メールアドレス更新開始', {
      ...
      after: maskEmail(newEmail)
    });
    if (process.env.NODE_ENV === 'development') console.log('更新予定のメールアドレス:', newEmail);
    ...
    if (process.env.NODE_ENV === 'development') console.log('✅ company_usersテーブルのメールアドレス更新成功!');
    ...
    try {
      ...
      if (authUpdateError) {
        ...
        if (process.env.NODE_ENV === 'development') console.warn('company_usersテーブルは更新済みですが、Supabase Authの更新に失敗しました');
      } else {
