...
export function NewPasswordClient({ userType: initialUserType }: NewPasswordClientProps) {
  ...
  useEffect(() => {
    try {
      ...
      // デバッグ情報を出力
      if (process.env.NODE_ENV === 'development') console.log('🔍 NewPasswordClient - Parameter analysis:', {
    ...
    async (password: string, confirmPassword: string) => {
      ...
      startTransition(async () => {
        try {
          ...
          const result: NewPasswordResult = await resetNewPasswordAction({
            tokenHash,
            type,
            accessToken,
            refreshToken,
          ...
          // その他のエラーは再スロー（NewPasswordFormでキャッチされる）
          throw error;
        }
      });
    },
    [allParams, parameterError, userType]
  );
    ...
    <div className='w-full'>
      {userType === 'candidate' ? (
        ...
        <div className='relative w-full max-w-[480px] md:max-w-[800px] bg-white rounded-[20px] md:rounded-[40px] shadow-[0px_0px_20px_0px_rgba(0,0,0,0.05)] px-6 md:px-[80px] py-10 md:py-[80px]'>
          <NewPasswordForm onSubmit={handleSubmit} isLoading={isPending} userType="candidate" />
        </div>
      ) : (
        // 企業ユーザーの場合
        <div className='relative w-full max-w-[480px] md:max-w-[800px] bg-white rounded-[20px] md:rounded-[10px] shadow-[0px_0px_20px_0px_rgba(0,0,0,0.05)] p-6 md:p-[80px]'>
          <NewPasswordForm onSubmit={handleSubmit} isLoading={isPending} userType="company" />
        </div>
      )}
    </div>
  );
