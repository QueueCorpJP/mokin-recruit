...
export default function SignupEducationPage() {
  ...
  const isFormValid = () => {
    return (
      ...
      formData.industries.every(
        industry => industry.experienceYears && industry.experienceYears !== ''
      ) &&
      formData.jobTypes.length > 0 &&
      formData.jobTypes.length <= 3 &&
      formData.jobTypes.every(
        jobType => jobType.experienceYears && jobType.experienceYears !== ''
      )
    );
  };
      ...
      const formattedData = {
        ...
        industries: formData.industries.map(industry => ({
          ...
          experienceYears: industry.experienceYears
            ? parseInt(industry.experienceYears.replace(/[^\d]/g, ''))
            : 0,
        })),
        jobTypes: formData.jobTypes.map(jobType => ({
          id: jobType.id,
          name: jobType.name,
          experienceYears: jobType.experienceYears
            ? parseInt(jobType.experienceYears.replace(/[^\d]/g, ''))
            : 0,
        })),
      };
    ...
    <div className='min-h-screen flex flex-col'>
      ...
      <div className='hidden lg:block'>
        ...
        <main
          ...
          style={{
            background:
              'linear-gradient(to top, #17856f, #229a4e), url("/back2.svg")',
