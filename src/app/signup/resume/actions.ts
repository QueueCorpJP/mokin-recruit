...
export async function uploadResumeFiles(formData: FormData) {
  if (process.env.NODE_ENV === 'development') console.log('=== Start uploadResumeFiles ===');
  try {
    // 環境変数の確認
    if (process.env.NODE_ENV === 'development') console.log('SUPABASE_URL:', process.env.NEXT_PUBLIC_SUPABASE_URL?.substring(0, 30) + '...');
    if (process.env.NODE_ENV === 'development') console.log('SUPABASE_KEY exists:', !!process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY);
    ...
    if (process.env.NODE_ENV === 'development') console.log('Supabase client created successfully');
    
    // Get or create candidate ID using the centralized function
    const candidateId = await getOrCreateCandidateId();
    if (process.env.NODE_ENV === 'development') console.log('Using candidate ID for resume upload:', candidateId);
    ...
    // 履歴書のアップロード
    if (resumeFile) {
      if (process.env.NODE_ENV === 'development') console.log('Uploading resume file:', resumeFile.name, resumeFile.size, resumeFile.type);
      const fileExt = resumeFile.name.split('.').pop();
      const fileName = `${candidateId}/resume_${Date.now()}.${fileExt}`;
      
      if (process.env.NODE_ENV === 'development') console.log('Target file path:', fileName);
      ...
      if (process.env.NODE_ENV === 'development') console.log('Resume upload result:', { data: resumeUploadData, error: resumeUploadError });
    ...
    if (process.env.NODE_ENV === 'development') console.log('Candidate data save result:', { data: candidateData, error: candidateError });

    if (candidateError) {
      throw new Error(`データベース保存に失敗: ${candidateError.message}`);
    }

    if (process.env.NODE_ENV === 'development') console.log(`アップロード成功: ${uploadResults.join(', ')}`);
    
    // 成功時のみ完了ページにリダイレクト
    redirect('/signup/resume/complete');
    
  } catch (error) {
    if (process.env.NODE_ENV === 'development') console.error('Resume upload error:', error);
    throw error;
  }
}