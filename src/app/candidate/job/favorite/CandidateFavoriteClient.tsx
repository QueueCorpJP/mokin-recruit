...
import { toJobCardDataFromFavoriteRow } from '@/lib/mappers/jobCard';
import Link from 'next/link';
import { useRouter, useSearchParams } from 'next/navigation';
import { removeFavoriteAction } from '@/lib/actions/favoriteActions';
...
export default function CandidateFavoriteClient({
  ...
  const transformedJobs =
    data?.favorites?.map((favorite: any) => {
      const card = toJobCardDataFromFavoriteRow(favorite);
  ...
  const handleStarClick = async (idx: number) => {
    ...
    try {
      ...
      const result = await removeFavoriteAction(job.id);
  ...
  const handlePageChange = (newPage: number) => {
    if (newPage >= 1 && newPage <= pagination.totalPages && !isPending) {
      ...
      startTransition(() => {
        ...
        router.replace(
          `/candidate/job/favorite?${newSearchParams.toString()}`,
          { scroll: false }
        );
      });
    }
