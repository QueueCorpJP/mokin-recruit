  ...
  try {
    ...
    if (resumeFile) {
      ...
      if (resumeUploadError) {
        throw new Error(
          `履歴書のアップロードに失敗しました: ${resumeUploadError.message}`
        );
      }
    ...
    if (careerSummaryFile) {
      ...
      const { data: careerUploadData, error: careerUploadError } =
      ...
      if (careerUploadError) {
        throw new Error(
          `職務経歴書のアップロードに失敗しました: ${careerUploadError.message}`
        );
      }
    ...
    // candidatesテーブルの履歴書情報を更新（URLではなくファイルパスを保存）
    const { data: candidateUpdateData, error: candidateError } = await supabase
      .from('candidates')
      .update({
        resume_url: resumeUrl, // This now stores the file path, not a public URL
    ...
    console.log('Candidate data save result:', {
    ...
    return {
      success: true,
      message: `${uploadResults.join('、')}をアップロードしました`,
