...
const InputRow = ({ label, value, isGrayBg = false }: InputRowProps) => (
  <div className='box-border content-stretch flex gap-2 items-start justify-start overflow-clip p-0 relative shrink-0 w-full'>
    <div
      className={`${
        isGrayBg ? 'bg-[#f9f9f9]' : 'bg-[#f9f9f9]'
      } box-border content-stretch flex flex-col gap-3 items-start justify-center min-h-[50px] px-6 py-3 relative rounded-[5px] self-stretch shrink-0 w-[200px]`}
    >
...
export default async function RirekisyoPreviewPage() {
  ...
  const formatEducation = () => {
    ...
    const month = educationData.graduation_month
      ? educationData.graduation_month.toString().padStart(2, '0')
      : 'mm';
    const schoolName = educationData.school_name || '学校名未設定';
    const department = educationData.department || '';

    return `${year}/${month}  ${schoolName} ${
      department ? `${department} ` : ''
    }${finalEdu ? `${finalEdu}卒業` : '卒業'}`;
  };
  ...
  const getCareers = () => {
    ...
    if (
      ...
      candidateData.recent_job_industries.forEach((job: any) => {
        const startPeriod = `${job.startYear || 'yyyy'}/${
          job.startMonth?.padStart(2, '0') || 'mm'
        }`;
        const endPeriod = job.isCurrentlyWorking
          ? '在籍中'
          : `${job.endYear || 'yyyy'}/${
              job.endMonth?.padStart(2, '0') || 'mm'
            }`;
        careers.push({
      ...
      // 単一職歴の場合（後方互換性）
      const startPeriod = `${candidateData.recent_job_start_year || 'yyyy'}/${
        candidateData.recent_job_start_month?.padStart(2, '0') || 'mm'
      }`;
      const endPeriod = candidateData.recent_job_is_currently_working
        ? '在籍中'
        : `${candidateData.recent_job_end_year || 'yyyy'}/${
            candidateData.recent_job_end_month?.padStart(2, '0') || 'mm'
          }`;
      careers.push({
        period: `${startPeriod}〜${endPeriod}`,
  ...
  const getOtherLanguages = () => {
    if (
      ...
      !Array.isArray(skillsData.other_languages)
    ) {
      return [];
    }
  ...
  const userData = {
    name: `${candidateData.last_name || '姓'} ${
      candidateData.first_name || '名'
    }`,
    ...
    address:
      candidateData.current_residence ||
      candidateData.prefecture ||
      '現在の住まい未設定',
