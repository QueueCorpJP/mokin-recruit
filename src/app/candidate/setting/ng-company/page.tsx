...
const closeIcon =
  'http://localhost:3845/assets/a0396fedb26c10cbda7f34a7019f04ee792845d6.svg';
const modalCloseIcon =
  'http://localhost:3845/assets/ca527250688df149a478765bdeb765225fed4f49.svg';
const tagCloseIcon =
  'http://localhost:3845/assets/20c8c5f9f2aa6159959eba646d0e3d86647e8627.svg';
...
function AddBlockCompanyModal({
  isOpen,
  onClose,
  onAddCompany,
  existingCompanies,
}: AddBlockCompanyModalProps) {
  ...
  const { suggestions: companySuggestions, loading: companyLoading } =
    useServerCompanyAutocomplete(companyName);

  const handleAddToSelected = (companyNameToAdd: string) => {
    if (
      companyNameToAdd.trim() &&
      !selectedCompanies.find(c => c.name === companyNameToAdd.trim())
    ) {
  ...
  return (
    <div className='min-h-screen bg-[#f9f9f9]'>
      ...
      <div className='bg-[#f9f9f9] box-border content-stretch flex flex-col gap-10 items-center justify-start pb-20 pt-10 px-4 md:px-20 relative w-full'>
        <div className='bg-[#ffffff] box-border content-stretch flex flex-col gap-10 items-start justify-start md:p-[40px] p-[24px] relative rounded-[10px] shrink-0 w-full'>
          <div className='box-border content-stretch flex flex-col gap-6 items-center justify-start p-0 relative shrink-0 w-full'>
            <div className="font-['Noto_Sans_JP:Bold',_sans-serif] leading-[0] not-italic relative shrink-0 text-[#323232] text-[16px] text-left tracking-[1.6px] font-bold w-full">
              <p className='block leading-[2]'>
                あなたの情報を公開したくない企業を設定することができます。
                <br aria-hidden='true' />
                現職や転職活動状況に登録した企業は自動で反映されますのでご安心ください。{' '}
              </p>
            </div>
          </div>
          <div className='box-border content-stretch flex flex-col md:flex-row gap-4 items-start justify-start p-0 relative shrink-0 w-full'>
            <div className='box-border content-stretch flex flex-col gap-2 items-start justify-start p-0 relative shrink-0 w-full max-w-[400px] md:w-[400px]'>
              <div className='box-border content-stretch flex flex-col gap-2 items-start justify-center p-0 relative shrink-0 w-full'>
        ...
        <div className='[flex-flow:wrap] box-border content-start flex gap-4 items-start justify-center p-0 relative shrink-0'>
          <Button
