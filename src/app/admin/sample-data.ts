...
export async function addSampleMediaArticles() {
  const supabase = createServerAdminClient();

  try {
    // Sample media articles data
    const articles = [
      {
      ...
      {
        title:
          'リモートワーク時代の面接対策：オンライン面接で好印象を与える方法',
    ...
    return {
      success: true,
      message: 'Sample media articles added successfully',
    };
  } catch (error) {
...
export async function addSampleNotices() {
  const supabase = createServerAdminClient();
  ...
  try {
    ...
    return { success: true, message: 'Sample notices added successfully' };
  } catch (error) {
    console.error('Error adding sample notices:', error);
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    };
