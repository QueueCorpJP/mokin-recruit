...
export default function SignupCareerStatusPage() {
  ...
  const methods = useForm({
    ...
    defaultValues: {
      hasCareerChange: undefined as 'yes' | 'no' | undefined,
      jobChangeTiming: '',
        ...
        {isDesktop ? (
          ...
          >
            ...
            <div className="bg-white rounded-[40px] shadow-[0px_0px_20px_0px_rgba(0,0,0,0.05)] p-20 w-[1000px] flex flex-col gap-10 items-center">
                  ...
                  <div className="flex flex-col gap-2 w-[400px]">
                    <div className="flex flex-row gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          setValue('hasCareerChange', 'no', {
                            shouldValidate: true,
                            shouldDirty: true,
                          });
                        }}
                        className={`flex-1 px-[11px] py-[11px] border rounded-[5px] text-[16px] font-bold tracking-[1.6px] transition-colors ${
                          selectedExperience === 'no'
                            ? 'bg-[#0f9058] border-[#0f9058] text-white'
                            : errors.hasCareerChange
                              ? 'bg-white border-red-500 text-[#999999] hover:border-[#0f9058]'
                              : 'bg-white border-[#999999] text-[#999999] hover:border-[#0f9058]'
                    ...
                    {errors.hasCareerChange && (
                      <p className="text-red-500 text-sm">
                        {errors.hasCareerChange.message}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex flex-row gap-4 items-start w-fit mx-auto mt-4">
                  ...
                  <div className="w-[400px]">
                    <div className="relative">
                      <select
                        {...register('jobChangeTiming')}
                        className={`w-full px-[11px] py-[11px] pr-10 bg-white border rounded-[5px] text-[16px] font-bold tracking-[1.6px] appearance-none cursor-pointer ${
                          errors.jobChangeTiming
                            ? 'border-red-500 text-[#323232]'
                            : 'border-[#999999] text-[#323232]'
                        }`}
                      >
                    ...
                    {errors.jobChangeTiming && (
                      <p className="text-red-500 text-sm mt-1">
                        {errors.jobChangeTiming.message}
                      </p>
                    )}
                  </div>
                </div>
              ...
              {/* Selection Status Entries - 条件付き表示 */}
              {currentActivityStatus !== 'not_started' &&
                currentActivityStatus !== 'researching' && (
                  <div className="flex flex-col gap-2 items-center w-[536]px">
                    ...
                    {fields.map((field, index) => (
                      ...
                      >
                        {/* Close button - only show for entries after the first one */}
                        {index > 0 && (
                          <div
                            ...
                            <div
                              ...
                              onClick={() => {
                                const currentValue = watch(
                                  `selectionEntries.${index}.isPrivate`,
                                );
                                setValue(
                                  `selectionEntries.${index}.isPrivate`,
                                  !currentValue,
                            ...
                            >
                              ...
                              >
                                <path
                                  ...
                                  fill={
                                    watch(`selectionEntries.${index}.isPrivate`)
                                      ? '#0F9058'
                                      : '#DCDCDC'
                                  }
                                />
                        ...
                        <div className="flex flex-row gap-4 items-start w-full">
                          ...
                          <input
                            ...
                            {...register(
                              `selectionEntries.${index}.department`,
                            )}
                            className="w-[400px] px-[11px] py-[11px] bg-white border border-[#999999] rounded-[5px] text-[16px] text-[#999999] font-medium tracking-[1.6px] placeholder:text-[#999999]"
                          />
                        </div>
                        ...
                        <div className="flex flex-row gap-4 items-start w-full">
                          ...
                          <div className="w-[400px]">
                            <div className="relative">
                              <select
                                {...register(
                                  `selectionEntries.${index}.progressStatus`,
                                )}
                        ...
                        {/* Decline Reason - 辞退選択時のみ表示 */}
                        {watch(`selectionEntries.${index}.progressStatus`) ===
                          'declined' && (
                          <div className="flex flex-row gap-4 items-start w-full">
                            <label className="text-[#323232] text-[16px] font-bold tracking-[1.6px] pt-[11px] min-w-[130px] text-right">
                              辞退理由
                            ...
                            <div className="w-[400px]">
                              <div className="relative">
                                <select
                                  {...register(
                                    `selectionEntries.${index}.declineReason`,
                                  )}
                                  className="w-full px-[11px] py-[11px] pr-10 bg-white border border-[#999999] rounded-[5px] text-[16px] text-[#323232] font-bold tracking-[1.6px] appearance-none cursor-pointer"
                                >
                                  <option value="">未選択</option>
                ...
                currentActivityStatus !== 'researching' && (
                  <div className="flex flex-col gap-2 items-center w-full">
                    ...
                    {fields.map((field, index) => (
                      ...
                      >
                        {/* Close button for entries after the first */}
                        {index > 0 && (
                          <div
      ...
      <IndustrySelectModal
        ...
        onConfirm={(industries) => {
          if (modalState.targetIndex !== null) {
            ...
            setSelectedIndustriesMap((prev) => {
              const newMap = { ...prev };
              newMap[targetIndex] = industries;
              return newMap;
            });
            // React Hook Formのフィールドも更新
            const fieldName =
              `selectionEntries.${modalState.targetIndex}.industries` as `selectionEntries.${number}.industries`;
            setValue(fieldName, industries, {
              shouldValidate: true,
              shouldDirty: true,
            });
          }
