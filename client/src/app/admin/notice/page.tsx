...
export default function NoticePage() {
  ...
  const getStatusBadge = (status: string) => {
    const statusClass = status === '公開中' 
      ? 'bg-[#0F9058] text-white' 
      : 'bg-gray-500 text-white';
    
    return (
      <span className={`inline-block px-3 py-1 rounded-[5px] font-['Inter'] text-[14px] font-bold leading-[1.6] tracking-[1.4px] ${statusClass}`}>
        {status}
      </span>
    );
  };
  ...
  const columns = [
    ...
    { key: 'datetime', label: '最終更新日時', sortable: true, width: 'w-[180px]' },
        ...
        <div className="mt-2 space-y-2">
          {paginatedData.map((notice) => (
            <AdminTableRow
              ...
              actions={[
                <ActionButton key="edit" text="編集" variant="edit" onClick={() => handleEdit(notice.id)} />,
                <ActionButton key="delete" text="削除" variant="delete" onClick={() => handleDelete(notice.id)} />
              ]}
            />
          ))}
        </div>
