...
export default function EditNoticeForm({ categories, saveNotice, initialNotice }: EditNoticeFormProps) {
  const router = useRouter();
  
  const [title, setTitle] = useState(initialNotice?.title || '');
  const [selectedCategoryIds, setSelectedCategoryIds] = useState<string[]>(() => {
    if (initialNotice?.notice_categories && Array.isArray(initialNotice.notice_categories)) {
  ...
  const [status, setStatus] = useState<'DRAFT' | 'PUBLISHED'>(initialNotice?.status || 'DRAFT');
  ...
  const [noticeId, setNoticeId] = useState<string | null>(initialNotice?.id || null);
  ...
  const handleThumbnailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      setThumbnailError('jpeg、jpg、png、gif形式のファイルのみアップロード可能です');
      return;
    }
