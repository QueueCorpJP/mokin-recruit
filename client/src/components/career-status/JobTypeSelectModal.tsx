...
export default function JobTypeSelectModal({
  ...
  const [selectedJobTypes, setSelectedJobTypes] =
    useState<JobType[]>(initialSelected);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      for (const group of JOB_TYPE_GROUPS) {
  ...
  // グループ内のすべての職種が選択されているかチェック
  const isGroupFullySelected = (groupId: string) => {
    const group = JOB_TYPE_GROUPS.find((g) => g.id === groupId);
    if (!group) return false;
    return group.jobTypes.every((jobType) =>
      selectedJobTypes.some((s) => s.id === jobType.id),
    );
  };

  // グループ内の一部の職種が選択されているかチェック
  const isGroupPartiallySelected = (groupId: string) => {
    const group = JOB_TYPE_GROUPS.find((g) => g.id === groupId);
    if (!group) return false;
    const selectedCount = group.jobTypes.filter((jobType) =>
      selectedJobTypes.some((s) => s.id === jobType.id),
    ).length;
    return selectedCount > 0 && selectedCount < group.jobTypes.length;
  };
  ...
  const handleGroupClick = (groupId: string) => {
    ...
    // 対象グループまでスクロール
    const targetGroup = document.getElementById(`jobtype-group-${groupId}`);
    if (targetGroup) {
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-[10px] w-[800px] h-[680px] flex flex-col">
            ...
            >
              ...
              {/* Job type groups and items */}
              <div className="space-y-10">
                {JOB_TYPE_GROUPS.map((group) => (
                  <div
                    key={group.id}
                    id={`jobtype-group-${group.id}`}
                    ref={(el) => {
        ...
        <div className="bg-white rounded-t-[24px] max-h-[90vh] flex flex-col">
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-x-0 gap-y-3 mb-4">
              {JOB_TYPE_GROUPS.slice(0, showAllCategories ? undefined : 6).map(
                (group, index) => (
                  <React.Fragment key={group.id}>
                    {index > 0 && (
                      <span className="mx-2 text-[#DCDCDC] text-[14px] font-bold">
            ...
            <div className="space-y-5">
              {(showAllCategories
                ? JOB_TYPE_GROUPS
                : JOB_TYPE_GROUPS.slice(0, 6)
              ).map((group) => (
                <div key={group.id} id={`jobtype-group-${group.id}`}>
